...
class CCL(nn.Module):
    ...
    def forward(self, im, s):
        ...
        # 使用 hardest negative：根据当前相似度选择每一行中概率最高的负样本
        n = scores.shape[0]
        device = scores.device
        eye = torch.eye(n, device=device, dtype=torch.bool)
        # 去掉对角线上的正样本
        i2t_neg = i2t.masked_fill(eye, 0.0)
        # 根据 ratio 决定每个 query 选择多少 hardest negatives
        num = n - 1 if self.ratio <= 0 or self.ratio >= 1 else int(self.ratio * n)
        num = max(1, min(num, n - 1))
        _, K = torch.topk(i2t_neg, k=num, dim=1)
        mask = torch.zeros_like(scores, device=device)
        row_idx = torch.arange(n, device=device).view(-1, 1)
        mask[row_idx, K] = 1.0
